...
Version 1.0, issued 3 August 2026. Status: awaiting sponsor and supervisor signature.
...
| Executive Sponsor | Sunil Kaushal — scope, content, budget, go-live. Sole Wix account owner and the only person able to publish or duplicate the site |
...
| O4 | A rebuilt public website on a duplicate of the live site | Redirect map covering every live URL; WCAG 2.2 AA; no inbound link or search ranking lost at cutover |
...
deliberately separate: it runs on a schedule against untrusted external sources, so it is kept out
of the request-serving application and constrained to draft output only. **It cannot distribute and
it cannot approve.**
...
| BR2 | No statistic, member count, board name or FTA detail is generated by the system. Sourced material only, with a visible placeholder where a fact is owed | A plausible invented figure is more dangerous than a blank, because nobody checks it. This already caught a homepage figure that would have published 95% of NZ's exports rather than 95% of its exports *to India* | Must |
...
|---|---|---|
| Phase 0 — SIP controlled launch, 27–31 July 2026 | All five runs recorded, source outcomes recorded, exceptions resolved or carried, QA completed daily, no unauthorised external publication, final launch review approved | Run; closeout and normal-production approval to be confirmed |
| Phase 1 — Assessment and foundation | Signed by Executive Sponsor, Finance Owner, Privacy Owner and Technical Lead | Substantially complete; gate not yet signed |
| Phase 2 — Core systems | Privacy, security, accessibility, data, UAT and recovery tests passed | In progress |
...
Sixteen weeks does not reach the end of Phase 4. The engagement delivers Phases 0 to 2 with the
four priority modules, opens Phase 3, and hands Phase 4 over as a specified backlog with its gates
already written. Detail in [the programme brief](./inzbc-ai-operating-system.md) §9.
...
| Stopping an illegal state change | A permissive default fails open: an unrecognised transition is allowed because nothing refused it. Reporting a lost race as an illegal move sends the operator to the wrong diagnosis | A fail-closed state machine with legal transitions only, hard human gates, a terminal stopped state, append-only transition history, and staleness checked before legality |
...
**Hours.** Approximately 20–40 hours per learner per week across the 16 weeks, recorded daily in
individual timesheets and derived from the commit record rather than estimated afterwards. Weeks 12
and 13 are deliberately unscheduled as float; every prior estimate on this project that assumed no
slippage has slipped.
...
- Every change reaches `main` through a reviewed pull request naming the issue it delivers. No
  direct pushes. Sensitive paths route to the technical lead automatically.
...
- Linting, type checking, tests, coverage, static analysis, secret scanning, dependency audit and
  workflow linting run on every pull request and must pass before merge.
...
- Cutover completed with no inbound link or search ranking lost, on the client's written authority.
