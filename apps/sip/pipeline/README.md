...
- `client.py` — REST client wrapping every pipeline endpoint in the API contract.
- `models.py` — request/response shapes mirroring `database/schema.sql`'s `runs`, `source_checks`,
  and `candidates` tables, including the shared enums.
- `tests/` — local checks that don't require a live server (currently: enum parity against the
  schema).
