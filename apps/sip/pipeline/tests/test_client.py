...
def test_create_run_sends_only_the_fields_create_run_in_accepts(monkeypatch):
    ...
    client = SipPipelineClient("http://x", "session-cookie", "csrf-token")
    ...
    # server-derived from the session (ADR-0004), not a caller-supplied field any more
    assert "initiated_by" not in sent["json"]


def test_create_candidate_never_sends_actor_id_or_unaccepted_defaulted_fields(monkeypatch):
    ...
    `actor_id` must not appear either: it is audit-only, and the server now derives it from the
    session (ADR-0004) instead of trusting a caller-supplied value.
    """
    sent: dict = {}
    ...
    client.create_candidate(Candidate(run_id="run-1", headline="Headline"))
    ...
    assert "actor_id" not in sent["json"]
