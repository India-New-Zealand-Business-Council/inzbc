...
export interface DailyBriefReport {
  id: string
  // `runs.id`, the UUID. Every write goes through this: `SubmitReportIn.run_id` and the
  // `/api/runs/{id}/...` transitions all validate it as one, so a run number here is a 500.
  //
  // It used to hold a run number, because the UI had no way to obtain a UUID -- the field was
  // doing display and identity at once and could only satisfy one. Now that a run is selected
  // from `GET /api/runs`, both values exist and each has its own field.
  runId: string
  // The human-readable run number ('RUN-SEED-06'), for anything a person reads. Never sent to the
  // API: no endpoint accepts one.
  runNumber: string
  ...
  state: RunState
  focusNote: string
  // Ids into the candidate list on screen, not the candidates themselves — mirrors how a real API
  // would
