...
// src/lib/notificationsData.ts. Shows only the most recent one; the full section below
// (#notifications) has the complete list. Two items (today's placeholder count) is exactly what
// #notifications holds too, so showing more than one here would just repeat that section verbatim
// above itself — the same reasoning applies once real notifications replace the placeholders.
export function NotificationsWidget() {
  return (
    <DashboardCard title="Notifications" linkHref="#notifications" linkLabel="View all notifications">
      <ul className="space-y-2">
        {PLACEHOLDER_NOTIFICATIONS.slice(0, 1).map((notification) => (
